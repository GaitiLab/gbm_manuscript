...
import libs.utils.helpers as helpers
import pandas as pd
import scanpy as sc
...
def get_args():
    ...
    parser.add_argument(
        "--gdf_barcodes_path",
        type=str,
        help="Path to GeoDataFrame with barcodes polygons",
    )
    parser.add_argument(
        "--gdf_segmentation_path",
        type=str,
        help="Path to GeoDataFrame with polygons of segmented cells/nuclei",
    )
...
def map_barcodes(
    ...
    barcodes_in_overlaping_polygons = pd.unique(
        result_spatial_join[result_spatial_join.duplicated(
            subset=["index"])]["index"]
    )
    ...
    (filtered_adata.obs).to_csv(
        Path(output_dir, f"{sample_id}__barcode_to_id_mapping.csv")
    )
    ...
    # TODO change output filename, is 'nuclei_seg' -> cell segmentation
    logging.info(
        "Saving nuclei binning results (geoDataFrame) as parquet file...")
    gdf_segmentation.to_parquet(
        os.path.join(output_dir, f"{sample_id}__nuclei_seg.parquet")
    )

    logging.info("Saving masks to json file for use in R...")
    gdf_segmentation.to_file(os.path.join(
        output_dir, f"{sample_id}__masks.json"), driver="GeoJSON")

    logging.info("Extract spatial information (centroids + polygons)...")
    coords = (
        gdf_segmentation["geometry"]
        .apply(lambda p: list(p.exterior.coords))
        .explode()
        .apply(pd.Series)
        .rename(columns=({0: "x", 1: "y"}))
    )

    # Extract centroids (centroids of polygons)
    gdf_segmentation["x_centroid"] = gdf_segmentation.centroid.x
    gdf_segmentation["y_centroid"] = gdf_segmentation.centroid.y

    # Convert to pandas
    gdf_pd = pd.DataFrame(gdf_segmentation).loc[
        :, ["id", "area", "x_centroid", "y_centroid"]
    ]

    gdf_combined = gdf_pd.merge(coords, left_index=True, right_index=True)
    logging.info("Save nucleus boundaries as .csv.gz file...")
    gdf_combined.to_csv(
        Path(output_dir, f"{sample_id}__nucleus_boundaries.csv.gz"),
        compression="gzip",
    )
    ...
    helpers.write_10X_h5(
        grouped_filtered_adata,
        str(
            Path(
                output_dir,
                f"{sample_id}__filtered_feature_bc_matrix_w_nuclei_seg.h5",
            )
        ),
    )
