...
findGeneMarkers <- function(
  ...
  results <- furrr::future_map(.x = genes, .f = function(gene) {
    ...
    for (id in unique(membership$grouping_var)) {
      members <- rownames(membership |> dplyr::filter(grouping_var == id))
      nonmembers <- rownames(membership |> dplyr::filter(grouping_var != id))
      tryCatch(
        {
          ...
          if (sum(!is.na(members_data)) < min_cells || sum(!is.na(nonmembers_data)) < min_cells) {
        ...
        error = function(e) {
          cat("Error processing gene:", gene, "and cluster:", id, ". Error: ", e$message, "\n")
          gene_results[[id]] <- NA
        }
      )
    }
  ...
  results <- results |> dplyr::group_by(grouping_var) |> dplyr::mutate(p.adj = stats::p.adjust(p.val, method = "bonferroni")) |>
...
amethyst_obj <- subsetObject(amethyst_obj, rownames(amethyst_obj@metadata)[amethyst_obj@metadata$CellClass == "Malignant"])

amethyst_obj@genomeMatrices[["gene_promoter_cg"]] <- makeWindows(amethyst_obj, genes = unique(amethyst_obj@ref$gene_name[amethyst_obj@ref$gene_type == "protein_coding"]),
    promoter = TRUE, type = "CG", metric = "percent", threads = 12, index = "chr_cg")
...
markers <- findGeneMarkers(amethyst_obj, 
                                    matrix = "gene_promoter_cg",
...
gsea_results <- fgsea::fgseaMultilevel(pathways = c2,
...
filter(padj < 0.5)
write.csv(df, file.path(base_plot_dir, paste0("fgseaRes_", pathway_set, "_gene_promoter_cg_pt_vs_te.csv")))
...
pathway_gobert <- gmt_import("GOBERT_OLIGODENDROCYTE_DIFFERENTIATION_UP.v2023.2.Hs.gmt")
pathway_kim <- gmt_import("KIM_ALL_DISORDERS_OLIGODENDROCYTE_NUMBER_CORR_UP.v2024.1.Hs.gmt")
pathway_neuro <- gmt_import("KEGG_NEUROACTIVE_LIGAND_RECEPTOR_INTERACTION.v2024.1.Hs.gmt")
sig_dn <- cluster_markers %>% filter(Delta < -2.5 & p.val < 0.05)
sig_up <- cluster_markers %>% filter(Delta > 2.5 & p.val < 0.05)

sig_genes_dn <- intersect(c(pathway_gobert[["GOBERT_OLIGODENDROCYTE_DIFFERENTIATION_UP"]], pathway_kim[["KIM_ALL_DISORDERS_OLIGODENDROCYTE_NUMBER_CORR_UP"]]), sig_dn$gene)
sig_genes_up <- intersect(pathway_neuro[["KEGG_NEUROACTIVE_LIGAND_RECEPTOR_INTERACTION"]], sig_up$gene)
sig_genes <- c(sig_genes_dn, sig_genes_up)

genes_to_label <- c(
  "CREB5", "CCDC61", "PEG3",
  "MARCKSL1", "FAM171A1", "MAST3", "RHOA",
  "GPR156", "GRM6", "CHRNA2", "GRIN1", "P2RX2", "HRH1"
)
cluster_markers$label <- ifelse(cluster_markers$gene %in% genes_to_label, cluster_markers$gene, NA)

cluster_markers$colour <- case_when(
    cluster_markers$gene %in% sig_genes & cluster_markers$gene %in% pathway_gobert[["GOBERT_OLIGODENDROCYTE_DIFFERENTIATION_UP"]] ~ "gobert",
    cluster_markers$gene %in% sig_genes & cluster_markers$gene %in% pathway_kim[["KIM_ALL_DISORDERS_OLIGODENDROCYTE_NUMBER_CORR_UP"]] ~ "kim",
    cluster_markers$gene %in% sig_genes & cluster_markers$gene %in% pathway_neuro[["KEGG_NEUROACTIVE_LIGAND_RECEPTOR_INTERACTION"]] ~ "neuroactive",
    TRUE ~ "black"
)
...
p <- ggplot(cluster_markers, aes(x = Delta, y = -log10(p.val))) +
  ...
  geom_label_repel(aes(label = label, colour = colour), na.rm = TRUE, size = 4, force = 1, nudge_y = 0.2, nudge_x = 0.2, max.overlaps = 15) +
  ...
  ylim(c(0,6)) +
  xlim(c(-10,10))
ggsave(filename = "pt_vs_te_volcano.pdf", path = plot_dir, width = 6, height = 8)
...
with(pd,
    ggplot(data=curve) +
        ...
        geom_ribbon(aes(x=rank, ymin=0, ymax=ES), fill="red4", alpha = 0.5) +
...
with(pd,
    ggplot(data=curve) +
        ...
        geom_ribbon(aes(x=rank, ymin=0, ymax=ES), fill="#7EBDC2", alpha = 0.5) +
        geom_ribbon(data=stats,
                    mapping=aes(x=rank, ymin=0,
                                ymax=stat/maxAbsStat*(spreadES/4)),
                    fill="grey") +
        geom_segment(data=ticks,
                    mapping=aes(x=rank, y=-spreadES/16,
                                xend=rank, yend=spreadES/16),
                    size=0.3) +
        geom_hline(yintercept=posES, colour="#7EBDC2", linetype="dashed") +
        geom_hline(yintercept=negES, colour="#7EBDC2", linetype="dashed") +
        geom_hline(yintercept=0, colour="black") +
        ylim(c(-0.2, 0.4))+
        theme(
            panel.background = element_blank(),
