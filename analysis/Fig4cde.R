# ---- Code to reproduce Figure 4c,d,e ---- #
...
params <- list(
    ...
    dmr_path = "GLM_TSSallcells_DATA_TSS_inv_high_Progenitor_like_GLM_MINCPGS_5_GLOBAL_inv_high_vs_pl_like.txt",
    gobert_oligo_diff_dn_gmt_path = "misc/internal/GOBERT_OLIGODENDROCYTE_DIFFERENTIATION_DN.v2023.2.Hs.gmt",
    gobert_oligo_diff_up_gmt_path = "misc/internal/GOBERT_OLIGODENDROCYTE_DIFFERENTIATION_UP.v2023.2.Hs.gmt",
    c2_cgp_gmt_path = "misc/internal/c2.cgp.v2023.2.Hs.symbols.gmt",
    c5_go_gmt_path = "misc/internal/c5.go.v2023.2.Hs.symbols.gmt",
...
pathway <- gmtPathways(
    params$gobert_oligo_diff_dn_gmt_path
)

sig_dn <- dmr %>% filter(DELTA < -0.05 & PVAL < 0.05)
sig_genes <- intersect(
    pathway[["GOBERT_OLIGODENDROCYTE_DIFFERENTIATION_DN"]],
    sig_dn$GENE
)
...
p <- ggplot(dmr, aes(x = DELTA * 100, y = -log10(PVAL))) +
    ...
    xlim(c(-50, 50))
ggsave(
    filename = "Fig4c_oligo_dn_volcano_inv_high_vs_pl_like.pdf",
...
gene_set <- gmtPathways(params$c2_cgp_gmt_path)
...
write.csv(df, file.path(params$output_dir, "Fig4d_fgseaRes.csv"))

pathway <- gmt_import(params$gobert_oligo_diff_dn_gmt_path)
...
with(
    ...
    ggplot(data = curve) +
        ...
        ggtitle(paste(
            "Invasive-high vs invasive-low OPC/NPC1 \n Gobert et al. Oligodendrocyte Differentiation Down"
        ))
)

ggsave(
    filename = "Fig4d_gobert_oligo_diff.pdf",
    path = params$plot_dir,
    width = 11
)
...
gene_set <- gmtPathways(params$c5_go_gmt_path)
...
pathway <- gmt_import(
    params$gobert_oligo_diff_up_gmt_path
)
pd <- plotEnrichmentData(
...
with(
    ...
    ggtitle(paste(
        "PT vs Tumour OPC/NPC1-like cells \n Oligodendrocyte Differentiation Up"
    ))
ggsave(filename = "Fig4e_oligo_diff_up_RNA.pdf", path = params$plot_dir)
