---
title: "Notebook to reproduce Figure S4a-d,f,h"
...
params:
  ...
  fgsea_res_c2_cp: "/fgseaRes_c2_cp.csv"
  neftel_sig: "/Neftel_gene_list.csv"
  deg_res: "/de_results.csv"
  gbmap_core: "/gbmap_core.rds"
  gbmap_meta: "/cluster/projects/gaitigroup/Users/Benson/Parsebio/output/Integrated_analysis/scVI/gbmap_inv_sig_metadata.csv"
...
## Figure S4a,b
```{r, figs4ab,  fig.height = 9, fig.width = 12}
# ---------------------------------------------------------------------------- #
#                                 Figure S4a,b                                 #
# ---------------------------------------------------------------------------- #

res <- read.csv(params$fgsea_res_c2_cp)
...
ggsave(filename = "c2_cp_select_pathways.pdf", path = params$plot_dir, height = 9, width = 12)
p
```
...
p <- ggplot(plot_df, aes(x = NES, y = reorder(pathway, NES))) +
  ...
  GBM_theme() +
  labs(
    title = "C2:CP Pathways Enriched in Tumour OPC/NPC1-like",
    x = "NES",
    y = "",
    colour = "-log10(FDR)"
  ) +
...
ggsave(filename = "c2_cp_select_pathways_dn.pdf", path = params$plot_dir, height = 9, width = 12)
p
```

## Figure S4c      
```{r, figs4c,  fig.width = 15, fig.height = 15 }
# ---------------------------------------------------------------------------- #
#                                  Figure S4c                                  #
# ---------------------------------------------------------------------------- #
# Load object
...
write.csv(degs, file.path(params$plot_dir, "invasive_sig_degs.csv"))

degs <- read.csv(file.path(params$plot_dir, "invasive_sig_degs.csv"), row.names = 1)
degs <- t(degs)
degs <- scale(degs)
degs <- t(degs)
...
genes_to_label <- c(
  "FGFR3", "DLL3", "FA2H", "LFNG", "BCAN", "DTX1", "NTRK3", "HES6", "EGFR", "GRIK1", "OLIG1", "NLGN3", "DLL1", "OLIG2",
  "CD44", "FOSL2", "NDRG1", "GLIS3", "FN1", "CHI3L1", "OSMR", "GAP43", "FOSL1"
)
index <- which(rownames(degs_sorted) %in% genes_to_label)
gene_labs <- rowAnnotation(foo = anno_mark(
  at = index,
...
pdf(file = paste0(params$plot_dir, "/invasive_sig_heatmap.pdf"), width = 15, height = 15)
...
## Figure S4d
```{r, figs4d, fig.width = 10, fig.height = 10}
# ---------------------------------------------------------------------------- #
#                                  Figure S4d                                  #
# ---------------------------------------------------------------------------- #

df <- read.csv(params$deg_res) # DE results from Fig1i
...
## Figure S4f
```{r, figs4f-1,  fig.height = 7, fig.width = 10}
# ---------------------------------------------------------------------------- #
#                                  Figure S4f                                  #
# ---------------------------------------------------------------------------- #

gbmap <- readRDS(params$gbmap_core) # path to gbmap obj
gbmap_genes <- rownames(gbmap@assays$RNA@counts)
write.csv(data.frame(gbmap_genes = gbmap_genes), file.path(params$plot_dir, "gbmap_genes.csv"))
...
write.csv(gbmap[[]], file.path(params$plot_dir, "gbmap_regional_samples.csv"))
...
df <- rbindlist(dfs)
write.csv(df, file.path(params$plot_dir, "gbmap_inv_sig_metadata.csv"))

plots <- wrap_plots(plots, ncol = 3, nrow = 1, guides = "collect")
ggsave(plots, filename = "inv_sig_gbmap.pdf", path = params$plot_dir, height = 7, width = 10)
plots
```

```{r, figs4f-2,  fig.height = 10, fig.width = 10}
gbmap <- read.csv(params$gbmap_meta)
...
ggsave(filename = "inv_sig_gbmap_paired.pdf", path = params$plot_dir, height = 10, width = 10)
p
```

## Figure S4h  
```{r, figs4h,  fig.width = 7, fig.height = 7}
# ---------------------------------------------------------------------------- #
#                                  Figure S4h                                  #
# ---------------------------------------------------------------------------- #
# Load object
...
ggsave(filename = "invasive_high_props.pdf", path = params$plot_dir, width = 7, height = 7)
p
```
