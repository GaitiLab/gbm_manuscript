...
colors <- GBMutils::load_color_palette("Spatial_CellClass_L2")
...
p_filled <- ggplot(polygons_df, aes(x = vertex_x, y = vertex_y)) +
    ...
    ggplot2::labs(
        x = "X (µm)", y = "Y (µm)",
        subtitle = glue::glue("No. of cells={prettyNum(nrow(meta), big.mark = ',')}")
    ) +
    scale_fill_manual(
        values = colors,
...
# EDIT: change pval
stats_res$neg_log10_padj <- -log10(p.adjust(stats_res$p_val, method = "bonferroni"))
