...
# TODO load archr_proj @Yiyan-YW check
...
ggsave(paste0(params$plot_dir, "/corGEM_MM_TF_Regulator_95.pdf"), width = 7, height = 7.5)
```


```{r, vis-2, fig.height = 80, fig.width = 5}
# ---------------------------- Scatter plot for each TF ---------------------------- #
p <- readRDS(paste0(params$plot_dir, "/TF_deviations.rds"))
...
exp_mat <- readRDS(paste0(params$plot_dir, "/TF_exp_mtx.csv"))
...
for (TF_name in unique(df_neuronal$TF)) {
  ...
  ggplot(curr_df, aes(x = mean_cell_type_gex, y = mean_cell_type_acc)) +
    ...
    ggtitle(TF_name)
  ggsave(paste0(params$plot_dir, "/Neuronal_motif_deviation_score_scatterplot_legend_", TF_name, ".pdf"), width = 5, height = 5.5)
}
```
