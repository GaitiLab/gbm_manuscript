...
motif_results <- setNames(
    lapply(motif_results_paths, function(x) {
        # Read the data
...
all_motif_results <- all_motif_results %>%
    mutate(
        motif = toupper(motif),
        target_sequence_with_motif = sub(
            "%",
            "",
            `% of Target Sequences with Motif`
        ),
        background_sequence_with_motif = sub(
            "%",
            "",
    ...
    mutate(
        ...
        background_sequence_with_motif = as.numeric(
            background_sequence_with_motif
        )
    ) %>%
    mutate(
        pct_diff = target_sequence_with_motif - background_sequence_with_motif
    ) %>%
    mutate(`P-value` = as.numeric(`P-value`), FDR = as.numeric(FDR))
...
diff_motif_results <- merge(
    ...
    mutate(rank = row_number()) %>%
    mutate(
        significant_status = case_when(
            p_value_OPC < 0.05 & p_value_COP > 0.05 & cell_type_pct_diff > 0 ~
                "Significant in OPC",
            p_value_OPC > 0.05 & p_value_COP < 0.05 & cell_type_pct_diff < 0 ~
                "Significant in COP",
...
p <- ggplot(diff_motif_results, aes(x = rank, y = cell_type_pct_diff)) +
    ...
    scale_fill_manual(
        values = c(
            "Significant in OPC" = "#ED8B22",
            "Significant in COP" = "#F0E9BA",
            "Significant in both" = "black",
            "Not significant" = "grey"
        )
    ) +
    scale_alpha_manual(
        values = c(
            "Significant in OPC" = 1,
            "Significant in COP" = 1,
            "Significant in both" = 1,
    ...
    ylim(c(
        -max(abs(diff_motif_results$cell_type_pct_diff), na.rm = TRUE),
        max(abs(diff_motif_results$cell_type_pct_diff), na.rm = TRUE)
    )) +
    theme_bw() +
    theme(
