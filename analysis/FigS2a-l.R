...
so_umap$umap <- ifelse(so_umap$is_malignant_confident == TRUE & so_umap$Region == "PT", "PT-Malignant", "Other")
p <- DimPlot(so_umap, reduction = "scVIumap", group.by = "umap", cols = c("PT-Malignant" = "black", "Other" = "grey"))
...
metadata <- so@meta.data %>% 
  ...
  so$is_malignant_confident == TRUE & so$Region == "PT" ~ "PT-Malignant",
  so$is_malignant_confident == TRUE & so$Region == "TE" ~ "TE-Malignant",
  so$is_malignant_confident == TRUE & so$Region == "TC" ~ "TC-Malignant",
  so$CellClass_L1 != "Malignant" ~ "Non-malignant"
))
so@meta.data <- metadata
so$vio <- factor(so$vio, levels = c("Non-malignant", "PT-Malignant", "TE-Malignant", "TC-Malignant"))
p <- VlnPlot(so, features = c("EGFR", "PTPRZ1"), pt.size = 0, group.by = "vio", cols = c("PT-Malignant" = "#7BB7AD", "TE-Malignant" = "#7BB7AD","TC-Malignant" = "#7BB7AD", "Non-malignant" = "grey"))
...
markers <- c("SLC1A2", "ADGRV1", "FLT1", "ABCB1", "EGFR", "PTPRZ1", "DOCK8", "APBB1IP", "CNTNAP2", "SYT1", "PLP1",
    "MBP", "PCDH15", "CA10", "IL7R", "SKAP1")
...
Idents(neurons) <- factor(Idents(neurons), levels = c("Glutamatergic", "GABAergic"))

markers <- c("SLC17A7","SLC17A6","GRIN1","GAD2","GAD1","SLC32A1")

p <- DotPlot(neurons, features = markers, dot.scale = 10, assay = "RNA", scale = TRUE) +
  coord_flip() +
  RotatedAxis()
...
p <- ggplot(df, aes(x = Region, y = n, fill = factor(CCI_CellClass_L2))) +
      ...
      theme(axis.text.x = element_blank(),
            ...
            legend.position = "bottom",
            legend.title = element_blank()) 
ggsave(filename = "myeloid_subtype_distribution.pdf", path = plot_dir, width = 5)

# Myeloid subtype markers
myeloid <- subset(so, subset = CellClass_L1 == "Myeloid")

Idents(myeloid) <- "CCI_CellClass_L2"
Idents(myeloid) <- factor(Idents(myeloid), levels = c("Myeloid_Immunosuppressive", "Myeloid_Inflammatory"))

markers <- c("C1QA", "TGFBI", "CD163", "CD83", "IL1B", "CCL3")

p <- DotPlot(myeloid, features = markers, dot.scale = 10, assay = "RNA", scale = TRUE) +
  coord_flip() +
  RotatedAxis()
...
a<-ggplot(df, aes(x = logneg, y = D))+
  ...
  geom_point(  aes(col=NPC) , data = subset(df, Type=="NPC") , size = 0.5, alpha = 1.0, stroke = NA)+
  scale_color_gradient2("NPC", low = "cadetblue1", mid = "cadetblue3" ,high = "navy")+
  new_scale("color")+
  geom_point(  aes(col=OPC), data = subset(df, Type=="OPC") , size = 0.5, alpha = 1.0, stroke = NA)+
  scale_color_gradient2("OPC", low = "darkseagreen1", mid = "darkseagreen3" ,  high = "darkolivegreen4")+
  new_scale("color")+
  geom_point(  aes(col=AC), data = subset(df, Type=="AC") , size = 0.5, alpha = 1.0, stroke = NA)+
  scale_color_gradient2("AC", low = "darkgoldenrod1",  mid= "darkgoldenrod2", high = "darkgoldenrod4")+
  new_scale("color")+
  geom_point(  aes(col=MES), data = subset(df, Type=="MES") , size = 0.5, alpha = 1.0, stroke = NA)+
  scale_color_gradient2("MES", low = "lavenderblush1", mid = "coral2", high = "darkred")+
  new_scale("color")+
  ...
  theme(plot.title = element_text(hjust = 0.5, color="black", family = "Helvetica"),
        axis.title.y = element_text(color="black", size=18, face = "bold", margin = margin(t = 0, r = 10, b = 0, l = 0), family = "Helvetica" ),
        axis.title.x = element_text(color="black", size=18, face = "bold", margin = margin(t = 0, r = 10, b = 0, l = 0), family = "Helvetica" ),
...
ggsave(filename = paste0("Neftel_quadrant_plot.pdf"), path = plot_dir, width = 15, height = 15)
# ---------------------------------------------------------------------------- #
#                                  Figure S2h                                  #
# ---------------------------------------------------------------------------- #
...
p <- ggplot(df, aes(x = Sample, y = n, fill = factor(CellClass_L3))) +
      ...
      theme(axis.text.x = element_blank(),
            ...
            legend.position = "bottom",
            legend.title = element_blank()) 
ggsave(filename = "cell_state_prop_by_sample.pdf", path = plot_dir, height = 25, width = 25)
...
Idents(malig_dotplot) <- factor(Idents(malig_dotplot), levels = c("Malignant_OPC", "Malignant_NPC1", "Malignant_NPC2", "Malignant_MES_INT", "Malignant_MES_AST", "Malignant_AC", "Malignant_MES_HYP"))

markers <- c("MKI67", "TNR", "BCAN", "OLIG2", "OLIG1", "DLL1", "DLL3", "SOX4", "SOX11", "TUBB3", "CD24",
    "VIM", "CD44", "CLU", "APOE", "AQP4", "GFAP", "NDRG1", "PKM", "ADM", "ATF3")

p <- DotPlot(malig_dotplot, features = markers, dot.scale = 8, assay = "RNA", scale = TRUE) +
    RotatedAxis()
ggsave(filename = "malignant_markers_dotplot.pdf", path = plot_dir, height = 5, width = 8)
...
invasivity <- read.csv("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/invasivity.csv")
...
connectivity <- read.csv("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/hai_connectivity.csv")
...
richards_markers <- read.csv(file.path("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/Richards_gene_list.csv"))
richards_markers <- na.omit(richards_markers) 

richards_gene_list <- list()
for(i in 1:nrow(richards_markers)) {
  richards_gene_list[[toString(richards_markers[i,"Group"])]] <- append(richards_gene_list[[toString(richards_markers[i,"Group"])]],toString(richards_markers[i, "Gene"]))
}
...
hypoxia_markers <- read.csv("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/hypoxia_markers.csv")
hypoxia_markers <- list(hypoxia_markers[, 2])
names(hypoxia_markers) <- "Hypoxia"

tnf_markers <- read.csv("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/tnf_alpha_markers.csv")
tnf_markers <- list(tnf_markers[, 2])
names(tnf_markers) <- "TNF_alpha"

emt_markers <- read.csv("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/EMT_markers.csv")
emt_markers <- list(emt_markers[,2])
names(emt_markers) <- "EMT"

infiltrating_margin <- c("ATP1A2", "FGFR3", "LMO3", "NCAN", "FXYD1", "PSD2", "PRODH", "HIF3A", "HRSP12","KCNN3",
                        "PPM1K", "KCNJ10", "ADCYAP1R1", "BMP7", "KAT2B", "CNTN1", "SAMD9L", "SLC7A11", "ECHDC2","FAM181B",
                        "SALL2", "SASH1") # Darmanis et al. 2017
infiltrating_margin <- list(infiltrating_margin)
names(infiltrating_margin) <- "Infiltration"

# Garofano
garofano <- read.csv("/cluster/projects/gaitigroup/Users/Benson/Parsebio/gene_lists/garofano.csv")
garofano <- as.vector(garofano)

# Combine gene signatures

genesets <- c(invasivity, richards_gene_list, hypoxia_markers, tnf_markers, emt_markers, connectivity,
              garofano["NEU"], infiltrating_margin)
...
score_df$Richards <- score_df$Richards_Developmental - score_df$Richards_Injury_Response
...
df <- as.data.table(so@meta.data)[, .(Region, Sample, Patient, CellClass_L3, CellClass_L2)]
...
for (gene_sig in colnames(score_df)) {
  ...
  dotplot_sample_level[[gene_sig]] <- ggplot(curr_data, aes(x = Region, y = Program)) +
  geom_boxplot(aes(fill = Region), outlier.shape = NA) + 
  scale_fill_manual(values = c(region_cols)) +
  geom_point(aes(group = Sample), size = 4, position = position_dodge(width = 0.75)) +
  GBM_theme() +
  xlab("") +
  ylab("Mean Normalized Module Score") +
  ggtitle(title) +
  stat_compare_means(label = "p.format", method = "wilcox.test", comparisons = comps) 

  
}

plots <- wrap_plots(dotplot_sample_level, ncol = 4, nrow = 5, guides = "collect") & theme(legend.position = "bottom")
ggsave(plots, filename = paste0("mod_score_gene_sig_boxplots_sample_level.pdf"), path = plot_dir, height = 25, width = 18)
