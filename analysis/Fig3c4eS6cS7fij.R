...
parser <- ArgumentParser(description = "Pipeline for SCENIC+ in silico perturbation simulation visualization.")
...
parser$add_argument("--merged_obj",
  type = "character",
...
parser$add_argument("--DE_genes",
  type = "character",
  default = "inv_sig.csv",
  help = "Differentially expressed genes."
)
parser$add_argument("--TF_to_perturb",
  type = "character",
  default = "ZEB1",
...
perturbation_dir <- paste0(args$output_dir, "/perturbation_simulation/", args$include_regions)
curr_seurat_obj <- readRDS(paste0(perturbation_dir, "/curr_seurat_obj_", args$TF_to_perturb, ".rds"))
...
#                                Figure 3c S6c                                 #
# ---------------------------------------------------------------------------- #
plot_invasion_probabilities_per_sample <- function(curr_seurat_obj, TF_to_perturb, perturbation_dir, annotation_column) {
  ...
  transition_prob_per_cell_type <- transition_prob %>%
    ...
    mutate(
      increased_prob = `Increased invasiveness` / (`Increased invasiveness` + `Decreased invasiveness`),
      decreased_prob = `Decreased invasiveness` / (`Increased invasiveness` + `Decreased invasiveness`)
    )

  # Calculate the p-value for each cell type
  binomial_results <- transition_prob_per_cell_type %>%
    mutate(success = ifelse(`Increased invasiveness` > `Decreased invasiveness`, `Increased invasiveness`, `Decreased invasiveness`)) %>%
    group_by(!!annotation_col) %>%
    summarize(
      observed_successes = sum(success == `Increased invasiveness`),
      total_trials = n(),
      p_value = binom.test(observed_successes, total_trials, p = 0.5, alternative = "two.sided")$p.value,
      .groups = "drop"
    ) %>%
    mutate(p_value = formatC(p_value, format = "e", digits = 2))
  binomial_results <- update_annotation_labels(binomial_results, annotation_col)

  # Transform the data to long format for plotting
  long_data <- transition_prob_per_cell_type %>%
    dplyr::select(!!annotation_col, Sample, increased_prob, decreased_prob) %>%
    pivot_longer(cols = c(increased_prob, decreased_prob), names_to = "Transition_Type", values_to = "Probability") %>%
  ...
  write.csv(summary_data, paste0(perturbation_dir, "/change_in_inv_per_sample_mean_", TF_to_perturb, "_", annotation_column, ".csv"))
  ...
  color_palette <- c("Increased invasiveness" = "#2B7095", "Decreased invasiveness" = "#7C9EB5")
  ...
  p <- ggplot(summary_data, aes(x = !!annotation_col, y = mean_prob, fill = Transition_Type)) +
    ...
    geom_text(
      ...
      aes(
        x = !!annotation_col, y = max(summary_data$mean_prob + summary_data$sem_prob) + 0.05,
        label = p_value
      ),
    ...
    labs(
      title = paste0("Change in invasiveness across cell states upon ", TF_to_perturb, " KO"),
      x = "Cell state",
  ...
  # Save the plot
  ggsave(paste0(perturbation_dir, "/change_in_inv_per_sample_", TF_to_perturb, "_", annotation_column, ".pdf"),
    plot = p,
...
plot_invasion_difference <- function(curr_seurat_obj, TF_to_perturb, perturbation_dir, annotation_column) {
  # Ensure the annotation column is a valid column in the Seurat object
  if (!annotation_column %in% colnames(curr_seurat_obj[[]])) {
    stop("The provided 'annotation_column' does not exist in the Seurat object.")
  } else {
  ...
  summary_data[[annotation_column]] <- factor(summary_data[[annotation_column]], levels = summary_data[[annotation_column]])

  ggplot(summary_data, aes(x = !!sym(annotation_col), y = mean_difference)) +
    geom_point(size = 3) + # Points for mean ratios
    geom_errorbar(aes(ymin = mean_difference - sem_difference, ymax = mean_difference + sem_difference),
      width = 0.2
    ) +
    ...
    scale_y_continuous( # Adjust y-axis limits and make it symmetric
      limits = c(-1, 1),
      expand = c(0, 0),
      labels = scales::percent_format(accuracy = 1)
    ) +
    GBM_theme()
  ggsave(paste0(perturbation_dir, "/invasion_difference_", TF_to_perturb, "_", annotation_column, ".pdf"),
    width = 8, height = 0.5 * length(unique(summary_data[[annotation_column]])) + 2
  )
}
...
plot_transition_probabilities_per_sample <- function(curr_seurat_obj, TF_to_perturb, perturbation_dir, annotation_column) {
  ...
  # Calculate the p-value for each cell type
  binomial_results <- transition_prob_per_cell_type %>%
    mutate(success = ifelse(Differentiation > `De-differentiation`, Differentiation, `De-differentiation`)) %>%
    group_by(!!annotation_col) %>%
    summarize(
  ...
  p <- ggplot(summary_data, aes(x = !!annotation_col, y = mean_prob, fill = Transition_Type)) +
    ...
    geom_errorbar(aes(ymin = mean_prob - sem_prob, ymax = mean_prob + sem_prob), 
                position = position_dodge(width = 0.7), width = 0.1, 
                color = "black", size = 0.5) +
    geom_text(data = binomial_results,
              aes(x = !!annotation_col, y = max(summary_data$mean_prob + summary_data$sem_prob) + 0.05, 
              label = p_value),
              inherit.aes = FALSE, size = 4, color = "black") +
    scale_fill_manual(values = color_palette) + 
    scale_y_continuous(labels = scales::percent_format(scale = 100)) +
    labs(title = paste0("Differentiation probability across cell states upon ", TF_to_perturb, " KO"),
         x = "Cell state",
...
plot_transition_difference <- function(curr_seurat_obj, TF_to_perturb, perturbation_dir, annotation_column) {
  # Ensure the annotation column is a valid column in the Seurat object
  if (!annotation_column %in% colnames(curr_seurat_obj[[]])) {
    stop("The provided 'annotation_column' does not exist in the Seurat object.")
  } else {
  ...
  transition_diffs <- transition_counts %>%
    ...
    mutate(prop_differece = (Differentiation - De_differentiation) / Total_n) %>%
    filter(Total_n >= 20) 
  ...
  ggplot(summary_data, aes(x = !!sym(annotation_col), y = mean_difference)) +
    ...
    GBM_theme()
  ggsave(paste0(perturbation_dir, "/transition_difference_", TF_to_perturb, "_", annotation_column, ".pdf"), 
                width = 8, height = 0.5 * length(unique(summary_data[[annotation_column]])) + 2)
}
...
#                                Figure S7ij                                   #
# ---------------------------------------------------------------------------- #
plot_signature_change_probabilities_per_sample <- function(curr_seurat_obj, TF_to_perturb, signature, perturbation_dir, annotation_column) {
  # Ensure the annotation column is a valid column in the Seurat object
  if (!annotation_column %in% colnames(curr_seurat_obj[[]])) {
    stop("The provided 'annotation_column' does not exist in the Seurat object.")
  } else {
  ...
  # Prepare the data with transition information
  all_cell <- curr_seurat_obj[[]] %>% 
    dplyr::select(!!original_sig, !!perturbed_sig, !!annotation_col, Sample) %>%
    mutate(transition = case_when(
  ...
  # Calculate the p-value for each cell type
  binomial_results <- transition_prob_per_cell_type %>%
    mutate(success = ifelse(Increased > Decreased, Increased, Decreased)) %>%
    group_by(!!annotation_col) %>%
    summarize(
  ...
  write.csv(summary_data, paste0(perturbation_dir, "/signature_transition_probability_per_sample_mean_", TF_to_perturb, "_", annotation_column, "_", signature, ".csv"))
  ...
  summary_data <- summary_data %>%
    mutate(sign_mean_prob = case_when(
      Transition_Type == "Increased" ~ mean_prob,
      Transition_Type == "Decreased" ~ -mean_prob
    ))
  p <- ggplot(summary_data, aes(x = !!annotation_col, y = sign_mean_prob, fill = Transition_Type)) +
      geom_bar(stat = "identity", position = "stack", width = 0.7) +
      geom_hline(yintercept = 0, linetype = "dashed") +
      geom_errorbar(aes(ymin = sign_mean_prob - sem_prob, ymax = sign_mean_prob + sem_prob), 
                    width = 0.2, 
                    color = "black", size = 0.5) +
      geom_text(data = binomial_results,
                aes(x = !!annotation_col, 
                    y = max(max(summary_data$sign_mean_prob + summary_data$sem_prob) + 0.05, 0.9),
                    label = p_value),
                inherit.aes = FALSE, size = 3, color = "black") +
      scale_fill_manual(values = color_palette) +
      labs(title = title,
           x = "Cell state",
           y = "Change in signature") +
      scale_y_continuous(
  ...
  ggsave(paste0(perturbation_dir, "/signature_transition_probability_per_sample_stacked_", TF_to_perturb, "_", annotation_column, "_", signature, ".pdf"), plot = p, 
         width = 1 * length(unique(summary_data[[annotation_column]])), height = 6)
}

# Plot signature change probabilities per sample
plot_signature_change_probabilities_per_sample(curr_seurat_obj = curr_seurat_obj, 
                                               TF_to_perturb = args$TF_to_perturb, 
                                               perturbation_dir = perturbation_dir, 
