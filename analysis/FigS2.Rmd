---
title: "Notebook to reproduce Figure S2"
...
# TODO @Bensonwu02 which functions are you using from here, that are not in GBMutils
...
## Figure S2a
```{r, figs2a, fig.height = 5, fig.width = 7}
# ---------------------------------------------------------------------------- #
#                                  Figure S2a                                  #
# ---------------------------------------------------------------------------- #
...
ggsave(p, filename = "scvi_pt_malignant_umap.pdf", path = params$plot_dir, height = 7, width = 9)
...
ggsave(filename = "PT_EGFR_PTPRZ1_expression.pdf", path = params$plot_dir, height = 5, width = 7)
p
```

## Figure S2b 
```{r, figs2b, fig.width = 8, fig.height = 5}
# ---------------------------------------------------------------------------- #
#                                  Figure S2b                                  #
# ---------------------------------------------------------------------------- #
...
markers <- c(
  "SLC1A2", "ADGRV1", "FLT1", "ABCB1", "DOCK8", "APBB1IP", "CNTNAP2", "SYT1", "PLP1",
  "MBP", "PCDH15", "CA10", "DCN", "EBF1", "IL7R", "SKAP1"
)

p <- DotPlot(so, features = markers, dot.scale = 8, assay = "RNA", scale = TRUE) +
  RotatedAxis()
ggsave(filename = "markers_dotplot.pdf", path = params$plot_dir, height = 5, width = 8)
p
```

## Figure S2c
```{r, figs2c, fig.height = 25, fig.width = 25}
# ---------------------------------------------------------------------------- #
#                                  Figure S2c                                  #
# ---------------------------------------------------------------------------- #
df <- so[[]] %>%
...
write.csv(df, file.path(params$plot_dir, "non_malig_abundance_by_sample.csv"))
...
ggsave(filename = "non_malig_cell_type_prop_by_sample.pdf", path = params$plot_dir, height = 25, width = 25)
p
```

## Figure S2d
```{r, figs2d, fig.width = 10, fig.height = 10}
# ---------------------------------------------------------------------------- #
#                                  Figure S2d                                  #
# ---------------------------------------------------------------------------- #
...
ggsave(filename = "tumour_cell_purity.pdf", path = params$plot_dir, width = 10, height = 10)
p
```

## Figure S2e 
```{r, figs2e, fig.height = 25, fig.width = 25}
# ---------------------------------------------------------------------------- #
#                                  Figure S2e                                  #
# ---------------------------------------------------------------------------- #
df <- so[[]] %>%
  ...
  summarise(n = n())
df$Region <- factor(df$Region, levels = c("PT", "TE", "TC"))
write.csv(df, file.path(params$plot_dir, "malignant_cell_num_by_sample.csv"))
...
ggsave(filename = "cell_type_prop_by_sample.pdf", path = params$plot_dir, height = 25, width = 25)
p
```

## Figure S2f 
```{r, figs2f, fig.width = 10, fig.height = 14}
# ---------------------------------------------------------------------------- #
#                                  Figure S2f                                  #
# ---------------------------------------------------------------------------- #
...
ggsave(plots, filename = "malignant_sample_cell_type_props.pdf", path = params$plot_dir, width = 10, height = 14)
plots
```

## Figure S2h      
```{r, figs2h, fig.height = 9, fig.width = 9}
# ---------------------------------------------------------------------------- #
#                                  Figure S2h                                  #
# ---------------------------------------------------------------------------- #
...
ggsave(plots, filename = "nonmalignant_sample_cell_type_props.pdf", path = params$plot_dir, height = 9, width = 9)
plots
```
