...
obj <- readRDS("path_to_amethyst_obj")
...
cnv_results <- cnvInference(obj = obj, 
...
pdf(file = file.path(plot_dir, "cnv_6425.pdf"), width = 15, height = 15)
cnv_results
dev.off()
...
p <- dimFeature(obj, colorBy = CellClass, reduction = "umap") +
  scale_color_manual(values = c("#87B5B1", "#E7298A", "#8C6D31", "#CCB883", "#E6AB02"))
ggsave(filename = paste0("cellclass_umap.pdf"), path = plot_dir)

# ---------------------------------------------------------------------------- #
#                                  Figure S8d                                  #
# ---------------------------------------------------------------------------- #

cellclass500bwindows <- calcSmoothedWindows(obj, 
                                        type = "CG", 
                                        threads = 10,
                                        step = 500,
                                        smooth = 3,
...
obj@genomeMatrices[["cg_cellclass_tracks"]] <- obj@genomeMatrices[["cg_cellclass_tracks"]][, !grepl("_t|_c", colnames(obj@genomeMatrices[["cg_cellclass_tracks"]])), with = FALSE]
pdf(file = paste0(plot_dir, "/markers_heatmap.pdf"), width = 15, height = 15)
heatMap(obj, 
        genes = c("GFAP", "AQP4", "C1QA", "P2RY13", "NEUROD6", "LINGO1", "MOG", "PLP1"), 
