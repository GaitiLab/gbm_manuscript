...
# Load seurat object
curr_seurat_data <- readRDS(seurat_obj_path)
...
curr_seurat_data <- subset(
    curr_seurat_data,
    subset = Confident_Annotation == TRUE
)
...
variable_genes_per_patient <- lapply(Split_object, function(patient_seurat) {
    patient_seurat <- FindVariableFeatures(
        patient_seurat,
...
variable_genes_per_patient_ranked <- lapply(
    variable_genes_per_patient,
    function(genes) {
        variable_genes_per_patient_ranked <- data.frame(
            gene = genes,
...
all_variable_genes_per_patient_ranked <- do.call(
    rbind,
    variable_genes_per_patient_ranked
)
...
curr_seurat_data$cell_type_per_patient <- paste0(
    curr_seurat_data$Patient,
    "__",
    curr_seurat_data$CellClass_L5_2
) # Get cell type per patient
...
pseudobulked_cell_type_per_patient_RNA <- AggregateExpression(
    curr_seurat_data,
    group.by = "cell_type_per_patient",
    assay = "RNA",
    return.seurat = TRUE
)
pseudobulked_cell_type_per_patient_RNA$CellClass_L5_2 <- sub(
    ".*__",
    "",
    rownames(pseudobulked_cell_type_per_patient_RNA[[]])
)

# Get the average gene expression per cell type
mean_cell_type_RNA <- as.data.frame(AverageExpression(
    pseudobulked_cell_type_per_patient_RNA,
...
# Format column names
colnames(mean_cell_type_RNA) <- sub(
    "Malignant_AC",
    "Malignant AC",
    colnames(mean_cell_type_RNA)
)
colnames(mean_cell_type_RNA) <- sub(
    "Malignant_MES_AST",
    "Malignant MES-AST",
...
write.csv(
    ...
    file.path(
        params$output_dir,
        paste0("FigS4i_correlation_matrix_", curr_param, "_genes.csv")
    )
)

# Heatmap of correlation matrix
correlation_df <- correlation_df[
    grepl("Malignant|Invasive", rownames(correlation_df)),
    grepl("Malignant|Invasive", colnames(correlation_df))
]
scaled_correlation_df <- (correlation_df - min(correlation_df)) /
    (max(correlation_df) - min(correlation_df))
...
pdf_output_path <- paste0(
    params$plot_dir,
    paste0(
        "FigS4i_correlation_heatmap_",
        genes,
        "_pseudobulked_patient_mean.pdf"
    )
)
pdf(pdf_output_path, width = 8.5, height = 8)
ht <- Heatmap(
    scaled_correlation_df,
    show_row_names = TRUE,
