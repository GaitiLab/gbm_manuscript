...
invasive_high_TFs <- read_csv("multiome_results/12_SCENIC_plus/outs/TFs_to_plot.csv")
invasive_high_TFs <- invasive_high_TFs$TF[invasive_high_TFs$Cell_type == "Invasive-high OPC/NPC1"]
lineage_gene_list[["Invasive-high TFs"]] <- invasive_high_TFs

lineage_gene_list <- lapply(lineage_gene_list, function(x) x[x %in% invasive_high_TFs])

output_file <- paste0(args$output_dir, "/Plots/venn_lineage.pdf")
pdf(output_file, width = 5, height = 5)
plot(euler(lineage_gene_list, shape = "ellipse"),
  quantities = TRUE
)
dev.off()
