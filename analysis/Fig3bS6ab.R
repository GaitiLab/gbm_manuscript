# ---- Code to reproduce Figure 3b and S6ab ---- #
...
plot_cor_TF_and_inv_per_cell <- function(
    ...
    if (length(grep("TCF12", colnames(chromVar_matrix_2))) == 0) {
        ...
        matched_names <- match_names(
            colnames(chromVar_matrix_2),
            chromVar_names
        )
    ...
    if (length(tf_name_matrix) == 0) {
        ...
        for (i in 1:length(tf_name_matrix)) {
            ...
            ggsave(
                paste0(
                    plot_dir,
                    "/Correlation_invasive_signature_ChromVar_per_cell_",
                    tf_name_matrix[i],
                    ".pdf"
                ),
                width = 4,
