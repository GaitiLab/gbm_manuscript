...
# ---- Setup script ---- #
# TODO @Yiyan-YW check if these are all the needed packages
pacman::p_load(Seurat, glue, data.table, tidyverse, stringr, ggplot2, Signac)

params <- list(
    plot_dir = "output/submission/figures",
    integrated_seurat_obj_path = "",
    merged_seurat_obj_path = "merged.rds"
)
...
integrated_seurat <- readRDS(params$integrated_seurat_obj_path)
...
metadata <- metadata %>%
    mutate(
        CellClass_L5_2 = case_when(
            ...
            CellClass_L5_2 == "Invasive-high OPC/NPC1" ~
                "Invasive-high OPC/NPC1"
        )
    )
curr_seurat_data <- AddMetaData(
    curr_seurat_data,
    metadata$CellClass_L5_2,
    col.name = "CellClass_L5_2"
)
...
curr_seurat_data$CellClass_L5_2 <- factor(
...
ggsave(
    filename = file.path(params$plot_dir, "FigS8f_CoveragePlot_DLL3.pdf"),
    plot = cov_plot,
    width = 10,
    height = 10
)
...
ggsave(
    filename = file.path(params$plot_dir, "FigS8f_AnnotationPlot_DLL3.pdf"),
    plot = gene_plot,
    width = 10,
    height = 10
)
...
expr_plot <- ExpressionPlot(
    ...
    scale_fill_manual(values = color_palette)
ggsave(
    filename = file.path(params$plot_dir, "FigS8f_ExpressionPlot_DLL3.pdf"),
    plot = expr_plot,
    width = 10,
    height = 10
)
...
ggsave(
    filename = file.path(params$plot_dir, "FigS8f_Combined_DLL3.pdf"),
    plot = combined,
    width = 7,
    height = 5
)
