---
title: "Notebook to reproduce Figure 2d"
...
params:
  ...
  path_to_meta_visium: "/visium_metadata.csv"
...
```{r, fig2d-1, fig.width = 10, fig.height = 10}
# Load DEGs
...
data_dirs <- list.dirs(params$path_to_greenwald, recursive = FALSE)

metadata <- read.csv(params$path_to_meta_visium)
...
greenwald_neuron <- foreach(sample = greenwald_neuron, .packages = c("dplyr")) %dopar% {
  ...
  adj_table <- GaitiLabUtils::build_adjacency_table(spot_pos_type)
  neuron_barcodes <- rownames(spot_pos_type)[spot_pos_type$mp == "neuron"]

  for (radius in seq(1, max_rad)) {
    neighbours_table <- GaitiLabUtils::find_neighbours_in_window(adj_table, radius)
...
for (sample in greenwald_neuron) {
  ...
  ggsave(filename = paste0(unique(sample$sample), "neuron_neighbour_3_dimplot.pdf"), path = params$plot_dir)
}
...
write.csv(plot_df, file.path(params$plot_dir, "plot_df.csv"))
...
ggsave(filename = "invasive_sig_neuron_colocal_rad_categorical.pdf", path = params$plot_dir)
p
```

```{r, fig2d-2, fig.width = 13, fig.height = 8}
# ---------------------------------------------------------------------------- #
...
ora <- function(samples, neighbour_res, annotation_col) {
  ...
  sample_p_vals <- foreach(sample = samples, .packages = c("dplyr")) %dopar% {
    ...
    relative_abundance <- metadata %>%
      ...
      ungroup() %>%
      mutate(
        total_neuron_prox = sum(mp_neighbour_total), relative_abundance = mp_neighbour_total / total_neuron_prox,
    ...
    write.csv(result_df, file.path(params$plot_dir, paste0(sample_name, "_ora_colocalization_", neighbour_res, ".csv")))
...
print(plot_df)
write.csv(plot_df, file.path(params$plot_dir, paste0("neuron_neighbour_", r, "_results.csv")))
...
ggsave(filename = "colocal_rank_plot.pdf", path = params$plot_dir, width = 13, height = 8)
p
```
