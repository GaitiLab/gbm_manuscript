...
# Find all CNV files
all_cnv_files <- list.files(args$output_dir, pattern = "CNV_signal.csv", full.names = TRUE, recursive = TRUE)
...
all_cnv_data$Chromosome <- factor(all_cnv_data$Chromosome, levels = c("Chr7", "Chr10"))

# Set gain and loss cutoffs 2 MADs away from the median for each chromosome
chr7gain_cutoff <- median(all_cnv_data$Scaled_CNV[all_cnv_data$Chromosome == "Chr7"]) + 2 * mad(all_cnv_data$Scaled_CNV[all_cnv_data$Chromosome == "Chr7"])
chr10loss_cutoff <- median(all_cnv_data$Scaled_CNV[all_cnv_data$Chromosome == "Chr10"]) - 2 * mad(all_cnv_data$Scaled_CNV[all_cnv_data$Chromosome == "Chr10"])
...
# Scatter plot
ggplot(scatter_df, aes(x = Chr7, y = Chr10)) +
  # Shade the areas for Chr7 gain and Chr10 loss
  annotate("rect", xmin = chr7gain_cutoff, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "red", alpha = 0.1) +  # Chr7 gain
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = chr10loss_cutoff, fill = "blue", alpha = 0.1) +  # Chr10 loss
  ...
  geom_text_repel(
    data = scatter_df[scatter_df$Chr7 > chr7gain_cutoff | scatter_df$Chr10 < chr10loss_cutoff, ],
    aes(label = Group),
    size = 3,
    color = "black",
    max.overlaps = Inf,
...
ggsave(paste0(args$output_dir, "/FigS5f_VisiumHC_CNV_signal.pdf"), width = 8, height = 6)
